import { formatCompact, getBatchCost, getPurchasePlan } from '../utils';
...
function UpgradeCard({ item, coins, quantity, actionLabel, onAction }) {
  ...
  const canAfford = quantity === 'max' ? plan.purchased > 0 : plan.purchased === quantity;
  ...
  return (
    <article className="wf-upgrade-card">
      ...
      <div className="wf-upgrade-info">
        ...
        {'level' in item ? <p>Level: {item.level}</p> : <p>Owned: {item.owned}</p>}
      </div>

      <div className="wf-upgrade-cta">
        <strong>{priceLabel}</strong>
        <button type="button" onClick={() => onAction(item.id)} disabled={!canAfford}>
          {buyLabel}
        </button>
      </div>
    </article>
